...
def load_model_pretrain(model):
    if cfg.PHASE == 'train':
        if cfg.TRAIN.PRETRAINED_MODEL_PATH:
            ...
            state_dict_to_load = torch.load(cfg.TRAIN.PRETRAINED_MODEL_PATH, weights_only=False)
            keys_wrong_shape = []
        ...
        if cfg.TEST.TEST_MODEL_PATH:
            if cfg.TEST.EPOCH: # load epoch model
                cfg.TEST.TEST_MODEL_PATH = cfg.TEST.TEST_MODEL_PATH.replace('.pth', '_epoch_{}.pth'.format(cfg.TEST.EPOCH))
