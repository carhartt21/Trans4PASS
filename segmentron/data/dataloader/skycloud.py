...
class SkyCloudSegmentation(SegmentationDataset):
    """Cityscapes Semantic Segmentation Dataset."""
    NUM_CLASS = 5

    def __init__(self, root='datasets/SkyCloud', split='train', mode=None, transform=None, **kwargs):
        super(SkyCloudSegmentation, self).__init__(root, split, mode, transform, **kwargs)
    ...
    def _val_sync_transform_resize(self, img, mask):
        w, h = img.size
        if w < self.crop_size[1] or h < self.crop_size[0]:
    ...
    def classes(self):
        """Category names."""
        return ('unlabeled', 'terrain', 'sky', 'thin cloud', 'thick cloud')
...
def _get_sky_pairs(folder, split='train'):
        ...
        for root, _, files in os.walk(img_folder):
            for filename in files:
                ...
                if filename.endswith('.jpg'):
                    ...
                    maskname = filename.replace('images', 'cloudlabels')
    ...
    mask_folder = os.path.join(folder, 'cloudlabels/')
    img_paths, mask_paths = get_path_pairs(img_folder, mask_folder)
    return img_paths, mask_paths

if __name__ == '__main__':
    dataset = SkyCloudSegmentation()
